...
- **i18n**: 各言語（英語、ドイツ語、イタリア語、中国語）の翻訳リソースを最新の文言に同期。
- **Maintenance**: バージョンメンテ。

### 1.20260503.14
- **Feature**: 言語設定（Language）を追加。システムの言語に加えて、日本語、英語、ドイツ語、イタリア語、中国語を任意に選択可能（再起動なしでUIに反映）。
- **Fix**: スニペット編集画面で項目を選択した際に発生していた `Publishing changes from within view updates` 警告と不安定な動作を、非同期バインディングの導入により完全に解消。
- **UI**: ステータスバーアイコンの「カラー」名称を「アプリアイコン」に変更し、デザイン（SVG）を刷新。ソリッド/ラインアイコンは背景適応（テンプレート）に戻し、視認性を向上。
- **Refactor**: プロジェクト全体のURLを `chromatri.be` に統一。
- **i18n**: Clipboard Typeなどのハードコードされていた文字列をローカライズファイルに移行。ドイツ語、イタリア語、中国語の翻訳ファイルを同期。
...
- **Modernization**: `DiagnosticServer` に Slack と Discord の両プラットフォームに対応した通知ロジックを実装。
- **Refactor**: 未使用のヘルパーメソッド（`menuItemTitle`）や変数（`mid`）を削除し、コードの健全性を向上。
